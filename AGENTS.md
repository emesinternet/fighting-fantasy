...
- **Adventure sheet controls:** The main grid tracks current Skill, Stamina, Luck, and Meals. Eating a meal restores 4 Stamina up to the max and decreases meals by 1. Gold, Treasures, and Equipment are free-form text areas for notes.
...
- **Saving and loading:** Save (left of New Game) prompts for a book page number, then downloads a JSON snapshot labeled with the page, the chosen book, and a readable timestamp. Load restores stats, maxima, potion state, enemies, notes, meals, and recent log history.
...
- **Book selection:** New games prompt for a Fighting Fantasy title before rolling stats. The chosen book is stored alongside saves and prefixes save filenames so runs stay organized.
